...
class ACO(object):
    ...
    def solve(self, graph: Graph):
        ...
        for gen in range(self.generations):
            print ('第{}代蚂蚁开始选择特征'.format(gen+1)+'。'*80)
            ants = [_Ant(self, graph, p0=0.2) for i in range(self.ant_count)]
            for ant in ants:
                if gen==0 and ants.index(ant)==0:
                    ...
                    print ('第0代第0只蚂蚁选择的特征：features_index_set={},features_status={}'.format(features_index_set,features_status))
                    print ('第0代第0只蚂蚁选择的特征:len(features_index_set)={},len(features_status)={}'.format(len(features_index_set),len(features_status)))
                    ...
                    features_index_set,features_status=ant._select_next_feature_status()
                    print ('len(features_index_set)={},len(features_status)={}'.format(len(features_index_set),len(features_status)))
...
class _Ant(object):
    ...
    def _select_next_feature_status(self):
        init_feature_status = list(np.random.rand(features_num))
        for j in range(len(init_feature_status)):
